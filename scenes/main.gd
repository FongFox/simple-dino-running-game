...
func _process(delta: float) -> void:
	...
	if game_running:
		...
		#print("Speed test: ", speed) # Test speed
		if speed > MAX_SPEED:
			speed = MAX_SPEED
		adjust_difficulty()
		# Generate obstacles
		...
		score += speed
		show_score()
		#print("Test score: ", score) # Print test score
...
func generate_obs() -> void:
	...
	if obstacles.is_empty() or last_obs.position.x < score + randi_range(300, 500):
		...
		for i in range(randi() % max_obs + 1):
			...
			var obs_x : int = screen_size.x + score + 350 + (i * 70)
		...
		if difficulty == MAX_DIFFICULTY:
			if(randi() % 2) == 0:
				...
				var obs_x : int = screen_size.x + score + 350
